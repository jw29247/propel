...
This document describes how the Propel ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`propel acp` exposes an ACP agent over stdio and forwards prompts to a running
Propel Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `propel acp` and speaks ACP messages over stdio.
...
`propel acp` resolves the Gateway URL and auth from CLI flags or config:
