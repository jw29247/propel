...
- Jobs persist under `~/.propel/cron/` so restarts don’t lose schedules.
...
propel cron list
propel cron run <job-id>
propel cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.propel/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `propel cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, Propel applies a
...
If `delivery` is omitted for isolated jobs, Propel defaults to `announce`.
...
- Job store: `~/.propel/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.propel/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
propel cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
propel cron edit <jobId> --agent ops
propel cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
propel cron run <jobId>
propel cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
propel cron edit <jobId> \
...
```bash
propel system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`propel system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `PROPEL_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Propel applies exponential retry backoff for recurring jobs after consecutive errors:
