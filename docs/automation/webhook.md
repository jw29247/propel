...
read_when:
  ...
  - Wiring external systems into Propel
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Propel config directory (typically `~/.propel/hooks/transforms`).
...
- `propel webhooks gmail setup` writes `hooks.gmail` config for `propel webhooks gmail run`.
