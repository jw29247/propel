...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Propel to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
propel plugins install @propel/feishu
```

Local checkout (when running from a git repo):

```bash
propel plugins install ./extensions/feishu
```
...
If you just installed Propel, run the wizard:
...
2. Configuring app credentials in Propel
...
1. You already ran `propel channels add` for Feishu
2. The gateway is running (`propel gateway status`)
...
```bash
propel pairing approve feishu <CODE>
```
...
2. Run `propel logs --follow` and look for `chat_id`
...
2. Run `propel logs --follow` and look for `open_id`
...
| `propel gateway restart` | Restart gateway service       |
| `propel logs --follow`   | Tail gateway logs             |
...
4. Check logs: `propel logs --follow`
...
5. Ensure the gateway is running: `propel gateway status`
6. Check logs: `propel logs --follow`
