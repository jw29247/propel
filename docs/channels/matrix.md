...
Matrix is an open, decentralized messaging protocol. Propel connects as a Matrix **user**
...
```bash
propel plugins install @propel/matrix
```

Local checkout (when running from a git repo):

```bash
propel plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `propel plugins install @propel/matrix`
   - From a local checkout: `propel plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Propel calls the same
     login endpoint, stores the access token in `~/.propel/credentials/matrix/credentials.json`,
...
- On first connection, Propel requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Propel logs a warning.
...
`~/.propel/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `propel pairing list matrix`
  - `propel pairing approve matrix <CODE>`
...
- On startup, Propel resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
