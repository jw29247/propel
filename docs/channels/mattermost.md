---
summary: "Mattermost bot setup and Propel config"
read_when:
...
```bash
propel plugins install @propel/mattermost
```

Local checkout (when running from a git repo):

```bash
propel plugins install ./extensions/mattermost
```
...
4. Configure Propel and start the gateway.
...
- Approve via:
  - `propel pairing list mattermost`
  - `propel pairing approve mattermost <CODE>`
...
Use these target formats with `propel message send` or cron/webhooks:
