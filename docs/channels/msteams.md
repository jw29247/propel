...
```bash
propel plugins install @propel/msteams
```

Local checkout (when running from a git repo):

```bash
propel plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
Propel will offer the local install path automatically.
...
3. Configure Propel with those credentials.
...
- Talk to Propel via Teams DMs, group chats, or channels.
...
- On startup, Propel resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.propel/propel.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Propel, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `propel-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `propel plugins install @propel/msteams`
   - From a local checkout: `propel plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "Propel in Teams", "full": "Propel in Teams" },
...
Propel handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Propel only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/PropelShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Propel sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `propel message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.propel/msteams-polls.json`.
...
```bash
propel message send --channel msteams \
...
propel message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
propel message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
propel message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
propel message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
