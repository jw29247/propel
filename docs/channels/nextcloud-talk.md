...
```bash
propel plugins install @propel/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
propel plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Propel" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `propel pairing list nextcloud-talk`
  - `propel pairing approve nextcloud-talk <CODE>`
