...
read_when:
  - You want Propel to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Propel to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`propel onboard`) and `propel channels add` list optional channel plugins.
...
```bash
propel plugins install @propel/nostr
```

Use a local checkout (dev workflows):

```bash
propel plugins install --link <path-to-propel>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "propel@example.com",
        "lud16": "propel@example.com"
      }
    }
  }
}
