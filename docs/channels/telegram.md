...
```
    ...
    Telegram does **not** use `propel channels login telegram`; configure token in config/env, then start gateway.
...
propel pairing approve telegram <CODE>
```
...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `propel doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
...
```
    ...
    - or read `chat.id` from `propel logs --follow`
...
- DM messages can carry `message_thread_id`; Propel routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    Propel can stream partial replies by sending a temporary Telegram message and editing it as text arrives.
    ...
    For text-only replies, Propel keeps the same preview message and performs a final edit in place (no second message).

    For complex replies (for example media payloads), Propel falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, Propel skips the preview stream to avoid double-streaming.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, Propel retries as plain text.
...
```
    ...
    - `~/.propel/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, Propel enqueues system events like:
...
```bash
propel message send --channel telegram --target 123456789 --message "hi"
propel message send --channel telegram --target @name --message "hi"
```
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `propel channels status` warns when config expects unmentioned group messages.
    - `propel channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `propel logs --follow` for skip reasons
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, Propel now retries these as recoverable network errors.
...
```
    ...
    - Environment overrides (temporary):
      - `PROPEL_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY=1`
      - `PROPEL_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY=1`
      - `PROPEL_TELEGRAM_DNS_RESULT_ORDER=ipv4first`
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `open` requires `"*"`. `propel doctor --fix` can resolve legacy `@username` entries to IDs.
- `channels.telegram.groupPolicy`: `open | allowlist | disabled` (default: allowlist).
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `propel doctor --fix` can resolve legacy `@username` entries to IDs.
