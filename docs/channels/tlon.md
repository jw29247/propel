...
Tlon is a decentralized messenger built on Urbit. Propel connects to your Urbit ship and can
...
```bash
propel plugins install @propel/tlon
```

Local checkout (when running from a git repo):

```bash
propel plugins install ./extensions/tlon
```
...
By default, Propel blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `propel message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Propel replies in-thread.
