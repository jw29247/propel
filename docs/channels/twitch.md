...
read_when:
  - Setting up Twitch chat integration for Propel
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Propel connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
propel plugins install @propel/twitch
```

Local checkout (when running from a git repo):

```bash
propel plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `PROPEL_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "propel", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use PROPEL_TWITCH_ACCESS_TOKEN env var)
...
```bash
PROPEL_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
