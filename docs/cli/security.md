---
summary: "CLI reference for `propel security` (audit and fix common security footguns)"
read_when:
...
It also warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `propel.browserConfigEpoch`) and recommends `propel sandbox recreate --browser --all`.
...
```bash
propel security audit --json | jq '.summary'
propel security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
propel security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
