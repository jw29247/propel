---
title: "Memory"
summary: "How Propel memory works (workspace files + automatic memory flush)"
read_when:
...
Propel memory is **plain Markdown in the agent workspace**. The files are the
...
`~/.propel/workspace`). See [Agent workspace](/concepts/agent-workspace) for the full layout.

## Memory tools

Propel exposes two agent-facing tools for these Markdown files:
...
When a session is **close to auto-compaction**, Propel triggers a **silent,
...
Propel can build a small vector index over `MEMORY.md` and `memory/*.md` so
...
- Configure memory search under `agents.defaults.memorySearch` (not top-level
  `memorySearch`).
- Uses remote embeddings by default. If `memorySearch.provider` is not set, Propel auto-selects:
...
Remote embeddings **require** an API key for the embedding provider. Propel
...
BM25 + vectors + reranking. Markdown stays the source of truth; Propel shells
...
- The gateway runs QMD in a self-contained XDG home under
  `~/.propel/agents/<agentId>/qmd/` by setting `XDG_CONFIG_HOME` and
  `XDG_CACHE_HOME`.
...
- The gateway writes a self-contained QMD home under
  `~/.propel/agents/<agentId>/qmd/` (config + cache + sqlite DB).
...
- Searches run via `memory.qmd.searchMode` (default `qmd search --json`; also
  ...
  QMD build, Propel retries with `qmd query`. If QMD fails or the binary is
  missing, Propel automatically falls back to the builtin SQLite manager so
  memory tools keep working.
- Propel does not expose QMD embed batch-size tuning today; batch behavior is
  controlled by QMD itself.
- **First search may be slow**: QMD may download local GGUF models (reranker/query
  expansion) on the first `qmd query` run.
  - Propel sets `XDG_CONFIG_HOME`/`XDG_CACHE_HOME` automatically when it runs QMD.
  - If you want to pre-download models manually (and warm the same index Propel
    uses), run a one-off query with the agent’s XDG dirs.

    Propel’s QMD state lives under your **state dir** (defaults to `~/.propel`).
    You can point `qmd` at the exact same index by exporting the same XDG vars
    Propel uses:

    ```bash
    # Pick the same state dir Propel uses
    STATE_DIR="${PROPEL_STATE_DIR:-$HOME/.propel}"
...
- When `scope` denies a search, Propel logs a warning with the derived
  `channel`/`chatType` so empty results are easier to debug.
- Snippets sourced outside the workspace show up as
  `qmd/<collection>/<relative-path>` in `memory_search` results; `memory_get`
  understands that prefix and reads from the configured QMD collection root.
- When `memory.qmd.sessions.enabled = true`, Propel exports sanitized session
  transcripts (User/Assistant turns) into a dedicated QMD collection under
  `~/.propel/agents/<id>/qmd/sessions/`, so `memory_search` can recall recent
...
- Index storage: per-agent SQLite at `~/.propel/memory/<agentId>.sqlite` (configurable via `agents.defaults.memorySearch.store.path`, supports `{agentId}` token).
- Freshness: watcher on `MEMORY.md` + `memory/` marks the index dirty (debounce 1.5s). Sync is scheduled on session start, on search, or on an interval and runs asynchronously. Session transcripts use delta thresholds to trigger background sync.
- Reindex triggers: the index stores the embedding **provider/model + endpoint fingerprint + chunking params**. If any of those change, Propel automatically resets and reindexes the entire store.

### Hybrid search (BM25 + vector)

When enabled, Propel combines:

- **Vector similarity** (semantic match, wording can differ)
- **BM25 keyword relevance** (exact tokens like IDs, env vars, code symbols)

If full-text search is unavailable on your platform, Propel falls back to vector-only search.
...
Propel can cache **chunk embeddings** in SQLite so reindexing and frequent updates (especially session transcripts) don't re-embed unchanged text.
...
- Session logs live on disk (`~/.propel/agents/<agentId>/sessions/*.jsonl`). Any process/user with filesystem access can read them, so treat disk access as the trust boundary. For stricter isolation, run agents under separate OS users or hosts.
...
When the sqlite-vec extension is available, Propel stores embeddings in a
...
- If the sqlite-vec extension is missing or fails to load, Propel logs the
