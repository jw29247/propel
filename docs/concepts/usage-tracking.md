...
- `/usage cost` in chats: local cost summary aggregated from Propel session logs.
- CLI: `propel status --usage` prints a full per-provider breakdown.
- CLI: `propel channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
