...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Propel
...
Running Propel via Node with `tsx` fails at startup with:

```
[propel] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `propel status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch propel.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node propel.mjs status` works on Node 25.
