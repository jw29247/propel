---
summary: "Research notes: offline memory system for Propel workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/.propel/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Propel integration
...
Target: Propel-style workspace (`agents.defaults.workspace`, default `~/.propel/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.propel/workspace` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want Propel to see every session.
...
```
~/.propel/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Propel**, but keep a separable core library.

### Why integrate into Propel?

- Propel already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `propel memory recall "…" --k 25 --since 30d`
  - `propel memory reflect --since 7d`
...
Pragmatic take for `~/.propel/workspace`:
