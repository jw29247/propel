...
description: "Complete field-by-field reference for ~/.propel/propel.json"
summary: "Complete reference for every Propel config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.propel/propel.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Propel uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `propel doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["propel-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `propel plugins install @propel/mattermost`.
...
Propel spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@propel"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `propel.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, Propel auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Propel assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.propel/sandboxes`
...
noVNC observer access uses VNC auth by default and Propel emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `propel-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.propel/workspace",
        agentDir: "~/.propel/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@propel"] },
        sandbox: { mode: "off" },
...
Propel uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.propel/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `PROPEL_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `propel onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `propel onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `propel onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `propel onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `propel onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `propel onboard --auth-choice minimax-api`.
...
- Loaded from `~/.propel/extensions`, `<workspace>/.propel/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or PROPEL_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `PROPEL_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.propel-dev` + port `19001`), `--profile <name>` (uses `~/.propel-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.propel/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-propel-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `PROPEL_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or PROPEL_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__propel__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__propel__/a2ui/`
...
- Also serves A2UI at `/__propel__/a2ui/`.
...
- Hostname defaults to `propel`. Override with `PROPEL_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.propel/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `propel dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.propel/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${PROPEL_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.propel/credentials/oauth.json`.
...
- Default log file: `/tmp/propel/propel-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `propel doctor --fix` can strip unknown keys).
