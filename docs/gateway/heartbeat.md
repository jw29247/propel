...
- During heartbeat runs, Propel treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `propel sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Propel skips the heartbeat run entirely (no model call).
...
```bash
propel system event --text "Check for urgent follow-ups" --mode now
```
