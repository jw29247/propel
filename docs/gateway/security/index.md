...
## Quick check: `propel security audit`
...
```bash
propel security audit
propel security audit --deep
propel security audit --fix
propel security audit --json
```

It flags common footguns (Gateway auth exposure, browser control exposure, elevated allowlists, filesystem permissions).

Propel is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
Propel assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.propel`, including `propel.json`), treat them as a trusted operator.
...
If you run `--deep`, Propel also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.propel/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.propel/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.propel/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.propel/credentials/oauth.json`
...
| -------------------------------------------------- | ------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                | critical      | Other users/processes can modify full Propel state                               | filesystem perms on `~/.propel`                                                                 | yes      |
| `fs.config.perms_writable`                         | critical      | Others can change auth/tool policy/config                                          | filesystem perms on `~/.propel/propel.json`                                                   | yes      |
...
`propel security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`propel security audit` includes `config.insecure_or_dangerous_flags` when any
...
gateway:
  ...
  auth:
    mode: password
    password: ${PROPEL_GATEWAY_PASSWORD}
```
...
Propel stores session transcripts on disk under `~/.propel/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.propel` (see the audit section below). If you need
...
Propel can refresh the skills list mid-session:
...
- If you install plugins from npm (`propel plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.propel/extensions/<pluginId>/` (or `$PROPEL_STATE_DIR/extensions/<pluginId>/`).
  - Propel uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
```bash
propel pairing list <channel>
propel pairing approve <channel> <code>
```
...
By default, Propel routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
Propel has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.propel/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.propel or your logs.”

## Unsafe external content bypass flags

Propel includes explicit bypass flags that disable external-content safety wrapping:
...
- `~/.propel/propel.json`: `600` (user read/write only)
- `~/.propel`: `700` (user only)

`propel doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `PROPEL_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__propel__/canvas/` and `/__propel__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_propel-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `PROPEL_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `propel doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `PROPEL_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `PROPEL_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Propel
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. Propel verifies the identity by resolving the
...
- Propel will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.propel/` (or `$PROPEL_STATE_DIR/`) may contain secrets or private data:

- `propel.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `propel status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@propel", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.propel/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `propel` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only Propel clients can connect.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default Propel limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `propel gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `PROPEL_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/propel/propel-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.propel/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `propel security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + Propel version
...
Found a vulnerability in Propel? Please report responsibly:

1. Email: [security@propel.ai](mailto:security@propel.ai)
