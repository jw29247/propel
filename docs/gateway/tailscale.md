...
Propel can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. Propel requires a shared password.
...
- `token` (default when `PROPEL_GATEWAY_TOKEN` is set)
- `password` (shared secret via `PROPEL_GATEWAY_PASSWORD` or config)

When `tailscale.mode = "serve"` and `gateway.auth.allowTailscale` is `true`,
Control UI/WebSocket auth can use Tailscale identity headers
(`tailscale-user-login`) without supplying a token/password. Propel verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
Propel only treats a request as Serve when it arrives from loopback with
...
Prefer `PROPEL_GATEWAY_PASSWORD` over committing a password to disk.
...
- Set `gateway.tailscale.resetOnExit` if you want Propel to undo `tailscale serve`
