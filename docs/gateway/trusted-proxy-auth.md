...
read_when:
  - Running Propel behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Propel
...
- You run Propel behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Propel checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Propel extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://propel.example.com
    to: http://propel-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`propel security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Propel config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `propel security audit` and review findings
