...
Deploy Propel on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/propel/propel)
...
repository configures everything needed to run Propel:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: PROPEL_STATE_DIR
        value: /data/.propel
      - key: PROPEL_WORKSPACE_DIR
        value: /data/workspace
      - key: PROPEL_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/propel`.
...
If you use the original Propel repository, Render will not auto-deploy your Propel. To update it, run a manual Blueprint sync from the dashboard.
...
For Propel, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Propel host.
