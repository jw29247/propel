...
service via the external `propel` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`PROPEL_PROFILE`; legacy `com.propel.*` is supported).
