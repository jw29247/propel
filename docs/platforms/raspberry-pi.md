---
summary: "Propel on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Propel on a Raspberry Pi
  - Running Propel on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Propel on Raspberry Pi

## Goal

Run a persistent, always-on Propel Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Propel features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled propel

# Enable if not
sudo systemctl enable propel

# Start on boot
sudo systemctl start propel
```
...
```bash
# Check logs
journalctl -u propel --no-pager -n 100

# Common fix: rebuild
cd ~/propel  # if using hackable install
npm run build
sudo systemctl restart propel
```
