---
summary: "Use MiniMax M2.1 in Propel"
read_when:
  - You want MiniMax models in Propel
...
```bash
propel plugins enable minimax-portal-auth  # skip if already loaded.
propel gateway restart  # restart if gateway is already running
propel onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/jw29247/propel/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `propel.json`:
...
- Use `propel models list` and `propel models set minimax/MiniMax-M2.1` to switch.
...
- Running `propel configure` and selecting **MiniMax M2.1**, or
