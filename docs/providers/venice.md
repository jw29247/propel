---
summary: "Use Venice AI privacy-focused models in Propel"
read_when:
  - You want privacy-focused inference in Propel
...
```bash
propel onboard --auth-choice venice-api-key
```
...
```bash
propel agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, Propel shows all available Venice models. Pick based on your needs:
...
```bash
propel models set venice/claude-opus-45
propel models set venice/llama-3.3-70b
```

List all available models:

```bash
propel models list | grep venice
```

## Configure via `propel configure`

1. Run `propel configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Propel automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
propel agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
propel agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
propel agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
propel agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
propel agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `propel models list` to see currently available models. Some models may be temporarily offline.
