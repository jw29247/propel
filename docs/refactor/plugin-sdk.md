...
- Publish as `propel/plugin-sdk` (or export from core under `propel/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `PropelPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: PropelConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: PropelConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: PropelConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: PropelConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: PropelConfig): string;
  };
};
```
...
- Introduce `propel/plugin-sdk`.
- Add `api.runtime` to `PropelPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `propelRuntime: ">=2026.2.0"`).
