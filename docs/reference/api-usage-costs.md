...
Propel features that can generate provider usage or paid API calls.
...
- `propel status --usage` and `propel channels list` show provider **usage windows**
...
Propel can pick up credentials from:
...
`propel models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
