...
Propel integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Propel owns lifecycle when `channels.signal.autoStart=true`.
...
- Propel spawns `imsg rpc` as a child process (legacy iMessage integration).
