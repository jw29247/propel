---
summary: "Chrome extension: let Propel drive your existing Chrome tab"
read_when:
...
The Propel Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate propel-managed Chrome profile.
...
Propel then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Propel release (npm package) as static files. There is no separate “build” step.

After upgrading Propel:

- Re-run `propel browser extension install` to refresh the installed files under your Propel state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

Propel ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `PROPEL_GATEWAY_TOKEN`)
...
- Open the tab you want Propel to control.
...
- `ON`: attached; Propel can drive that tab.
...
Debugging: `propel sandbox explain`
...
`propel browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `propel browser extension install` first to copy the extension to a stable location under your Propel state directory.
...
- **This is not isolated** like the dedicated propel-managed profile.
