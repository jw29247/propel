...
- **gateway host** → `propel` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.propel/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.propel/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `propel approvals allowlist ...`).
- `propel security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `propel doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.propel/exec-approvals.json` directly.

CLI: `propel approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
