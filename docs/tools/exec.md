...
- `gateway`/`node` approvals are controlled by `~/.propel/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Propel prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`propel security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `propel doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
