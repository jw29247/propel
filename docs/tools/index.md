---
summary: "Agent tool surface for Propel (browser, canvas, nodes, message, cron) replacing legacy `propel-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `propel-*` skills
title: "Tools"
---

# Tools (Propel)

Propel exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `propel-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `propel.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Propel logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:propel`: all built-in Propel tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`propel node run`).
...
Propel tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `propel configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Propel-managed browser.
...
- Quick smoke: `propel nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `propel gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, Propel clamps `tools.sessions.visibility` to `tree`.
