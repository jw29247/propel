...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `propel sandbox explain`.
...
Legacy `agent.*` configs are migrated by `propel doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=propel-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${PROPEL_STATE_DIR:-$HOME/.propel}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
