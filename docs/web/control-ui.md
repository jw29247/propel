...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/propel`)
...
If the page fails to load, start the Gateway first: `propel gateway`.
...
you revoke it with `propel devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.propel/propel.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Propel
...
Propel **blocks** Control UI connections without device identity.
