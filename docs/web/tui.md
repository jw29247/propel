...
```bash
propel tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `propel tui --deliver`
...
- Check the Gateway logs: `propel logs --follow`.
- Confirm the agent can run: `propel status` and `propel models status`.
...
- No agents in picker: check `propel agents list` and your routing config.
