...
Propel 使用 Bonjour（mDNS / DNS‑SD）作为**仅限局域网的便捷方式**来发现
...
2. 在专用区域下发布 `_propel-gw._tcp` 的 DNS‑SD 记录
   （示例：`propel.internal.`）。
3. 配置 Tailscale **分割 DNS**，使你选择的域名通过该
   DNS 服务器为客户端（包括 iOS）解析。

Propel 支持任何发现域名；`propel.internal.` 只是一个示例。
...
- 从 `~/.propel/dns/<domain>.db` 提供你选择的域名服务（示例：`propel.internal.`）
...
你的发现域名中浏览 `_propel-gw._tcp`，无需多播。
...
- 在 `~/.propel/propel.json` 中设置 `gateway.bind: "tailnet"`。
- 重启 Gateway 网关（或重启 macOS 菜单栏应用）。

## 什么在广播

只有 Gateway 网关广播 `_propel-gw._tcp`。

## 服务类型

- `_propel-gw._tcp` — Gateway 网关传输信标（被 macOS/iOS/Android 节点使用）。
...
- `transport=gateway`
- `cliPath=<path>`（可选；可运行的 `propel` 入口点的绝对路径）
...
iOS 节点使用 `NWBrowser` 来发现 `_propel-gw._tcp`。
...
- `PROPEL_DISABLE_BONJOUR=1` 禁用广播（旧版：`PROPEL_DISABLE_BONJOUR`）。
- `~/.propel/propel.json` 中的 `gateway.bind` 控制 Gateway 网关绑定模式。
- `PROPEL_SSH_PORT` 覆盖 TXT 中广播的 SSH 端口（旧版：`PROPEL_SSH_PORT`）。
- `PROPEL_TAILNET_DNS` 在 TXT 中发布 MagicDNS 提示（旧版：`PROPEL_TAILNET_DNS`）。
- `PROPEL_CLI_PATH` 覆盖广播的 CLI 路径（旧版：`PROPEL_CLI_PATH`）。
