...
你可以在 `~/.propel/propel.json` 中覆盖此设置：
...
所有日志配置都在 `~/.propel/propel.json` 的 `logging` 下。
...
- Propel 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `propel plugins enable diagnostics-otel` 启用插件。
...
- `propel.tokens`（计数器，属性：`propel.token`、`propel.channel`、`propel.provider`、`propel.model`）
- `propel.cost.usd`（计数器，属性：`propel.channel`、`propel.provider`、`propel.model`）
- `propel.run.duration_ms`（直方图，属性：`propel.channel`、`propel.provider`、`propel.model`）
- `propel.context.tokens`（直方图，属性：`propel.context`、`propel.channel`、`propel.provider`、`propel.model`）

消息流：

- `propel.webhook.received`（计数器，属性：`propel.channel`、`propel.webhook`）
- `propel.webhook.error`（计数器，属性：`propel.channel`、`propel.webhook`）
- `propel.webhook.duration_ms`（直方图，属性：`propel.channel`、`propel.webhook`）
- `propel.message.queued`（计数器，属性：`propel.channel`、`propel.source`）
- `propel.message.processed`（计数器，属性：`propel.channel`、`propel.outcome`）
- `propel.message.duration_ms`（直方图，属性：`propel.channel`、`propel.outcome`）

队列 + 会话：

- `propel.queue.lane.enqueue`（计数器，属性：`propel.lane`）
- `propel.queue.lane.dequeue`（计数器，属性：`propel.lane`）
- `propel.queue.depth`（直方图，属性：`propel.lane` 或 `propel.channel=heartbeat`）
- `propel.queue.wait_ms`（直方图，属性：`propel.lane`）
- `propel.session.state`（计数器，属性：`propel.state`、`propel.reason`）
- `propel.session.stuck`（计数器，属性：`propel.state`）
- `propel.session.stuck_age_ms`（直方图，属性：`propel.state`）
- `propel.run.attempt`（计数器，属性：`propel.attempt`）
...
- `propel.webhook.error`
  - `propel.channel`、`propel.webhook`、`propel.chatId`、`propel.error`
- `propel.message.processed`
  - `propel.channel`、`propel.outcome`、`propel.chatId`、`propel.messageId`、`propel.sessionKey`、`propel.sessionId`、`propel.reason`
- `propel.session.stuck`
  - `propel.state`、`propel.ageMs`、`propel.queueDepth`、`propel.sessionKey`、`propel.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `propel doctor`。
