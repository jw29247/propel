---
read_when:
  - 你想在 Propel 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 Propel 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
propel plugins enable minimax-portal-auth  # 如果已加载则跳过
propel gateway restart  # 如果 Gateway 网关已在运行则重启
...
详情参见 [MiniMax OAuth 插件 README](https://github.com/jw29247/propel/tree/main/extensions/minimax-portal-auth)。
...
- 使用 `propel models list` 和 `propel models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `propel configure` 并选择 **MiniMax M2.1**，或
