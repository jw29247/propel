...
import { createDedupeCache, createPersistentDedupe } from "propel/plugin-sdk";
...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  const stateOverride = env.PROPEL_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
  if (stateOverride) {
    return stateOverride;
  }
  if (env.VITEST || env.NODE_ENV === "test") {
    return path.join(os.tmpdir(), ["propel-vitest", String(process.pid)].join("-"));
  }
  return path.join(os.homedir(), ".propel");
}
