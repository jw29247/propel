...
`propel.invoke --each`) without adding new Propel code per workflow.
...
This extension depends on Propel internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Propel extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.propel/extensions` as a standalone plugin directory.
