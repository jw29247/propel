...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["propelPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["propel", "pollId"]) ??
    readNestedString(value, ["propel", "poll", "id"]) ??
    readNestedString(value, ["data", "propelPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "propel", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "propel poll vote",
          displayText: "Vote recorded",
          value: { propelPollId: pollId, pollId },
        },
      },
    ],
  };
