...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as PropelConfig);
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as PropelConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as PropelConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as PropelConfig);
