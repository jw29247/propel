...
metadata: { "propel": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Propel `sessions_spawn`
- **File I/O** == Propel `read`/`write`
- **Remote fetch** == Propel `web_fetch` (or `exec` with curl when POST is required)
