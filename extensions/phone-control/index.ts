...
import type { PropelPluginApi, PropelPluginService } from "propel/plugin-sdk";
...
function normalizeDenyList(cfg: PropelPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: PropelPluginApi["config"]): string[] {
...
export default function register(api: PropelPluginApi) {
  ...
  const timerService: PropelPluginService = {
