...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: PROPEL_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: PropelConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var PROPEL_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.PROPEL_TWITCH_ACCESS_TOKEN?.trim();
