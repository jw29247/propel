/**
 * Twitch channel plugin for Propel.
 *
...
import { buildChannelConfigSchema } from "propel/plugin-sdk";
 ...
 * for Propel. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: PropelConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: PropelConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: PropelConfig): boolean => {
