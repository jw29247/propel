...
function resolvePropelRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.PROPEL_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set PROPEL_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolvePropelRoot(), "dist", "extensionAPI.js");
