...
Propel extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
propel channels login --channel zalouser
...
```bash
propel message send --channel zalouser --target <threadId> --message "Hello from Propel!"
```
...
```bash
propel channels login --channel zalouser              # Login via QR
propel channels login --channel zalouser --account work
propel channels status --probe
propel channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
propel directory self --channel zalouser
propel directory peers list --channel zalouser --query "name"
propel directory groups list --channel zalouser --query "work"
propel directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
propel message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
propel message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `propel channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
propel channels login --channel zalouser --account work
propel message send --channel zalouser --account work --target <id> --message "Hello"
