...
#
# Suggested cron: */30 * * * * /home/admin/propel/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Propel notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., propel-alerts)
...
STATE_FILE="$HOME/.propel/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Propel if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use propel
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Propel to $NOTIFY_PHONE..."
            propel send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Propel is down. Run: ssh l36 '~/propel/scripts/mobile-reauth.sh'" "urgent"
    exit 1
