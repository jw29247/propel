...
DMG_TEMP="$(mktemp -d /tmp/propel-dmg.XXXXXX)"
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/propel-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/propel-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/propel-dmg-limits.txt
