...
INSTALL_URL="${PROPEL_INSTALL_URL:-https://propel.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${PROPEL_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${PROPEL_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="propel"
PACKAGE_NAME="${PROPEL_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (PROPEL_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${PROPEL_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${PROPEL_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
