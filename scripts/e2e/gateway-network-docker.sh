...
IMAGE_NAME="propel-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="propel-net-e2e-$$"
GW_NAME="propel-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "PROPEL_GATEWAY_TOKEN=$TOKEN" \
	  -e "PROPEL_SKIP_CHANNELS=1" \
	  -e "PROPEL_SKIP_GMAIL_WATCHER=1" \
	  -e "PROPEL_SKIP_CRON=1" \
	  -e "PROPEL_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
