...
const shouldBuild = (deps) => {
  if (deps.env.PROPEL_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.PROPEL_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[propel] ${message}\n`);
};

const runPropel = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["propel.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
