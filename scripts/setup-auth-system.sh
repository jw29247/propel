#!/bin/bash
# Setup Propel Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'propel-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Propel message: Send warning via Propel itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/propel-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/propel-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/propel-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now propel-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status propel-auth-monitor.timer"
echo ""
echo "Quick commands:"
