...
Inspired by Simon Willison's [Running Propel in Docker](https://til.simonwillison.net/llms/propel-docker).
...
On first command, ClawDock auto-detects your Propel directory:

- Checks common paths (`~/propel`, `~/workspace/propel`, etc.)
...
| `clawdock-cli <command>`  | Run Propel CLI commands                      |
...
| `clawdock-cd`        | Jump to the Propel project directory    |
| `clawdock-config`    | Open the Propel config directory        |
...
```bash
propel channels login --channel whatsapp --verbose
```
...
2. Configure it in the Propel config
...
- Bash or Zsh shell
- Propel project (from `docker-setup.sh`)
