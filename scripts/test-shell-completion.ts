 ...
 * `propel update`. Use it to verify the completion installation flow
...
\`propel update\`, \`propel doctor\`, and \`propel onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Propel Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`propel completion --install\` later to enable.`));
    return;
  }
