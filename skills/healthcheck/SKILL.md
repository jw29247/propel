---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Propel deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Propel cron scheduling for periodic checks, or version status checks on a machine running Propel (laptop, workstation, Pi, VPS).
---

# Propel Host Hardening

## Overview

Assess and harden the host running Propel, then align it to a user-defined risk tolerance without breaking access. Use Propel security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Propel changes the host firewall, SSH, or OS updates; it does not.
...
5. Propel gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Propel can access sensitive data.
...
### 2) Run Propel security audits (read-only)

As part of the default read-only checks, run `propel security audit --deep`. Only offer alternatives if the user requests them:

1. `propel security audit` (faster, non-probing)
2. `propel security audit --json` (structured output)

Offer to apply Propel safe defaults (numbered):

1. `propel security audit --fix`

Be explicit that `--fix` only tightens Propel defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Propel version/update status (read-only)

As part of the default read-only checks, run `propel update status`.
...
- Credential hygiene notes (location of Propel creds, prefer disk encryption)
...
After Propel install or first hardening pass, run at least one baseline audit and version check:

- `propel security audit`
- `propel security audit --deep`
- `propel update status`

Ongoing monitoring is recommended. Use the Propel cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `propel cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `propel update status`
...
Before creating, `propel cron list` and match on exact `name`. If found, `propel cron edit <id> ...`.
If not found, `propel cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `propel update status` (preferred for source checkouts and channels)
2. `npm view propel version` (published npm version)

## Propel command accuracy

Use only supported commands and flags:

- `propel security audit [--deep] [--fix] [--json]`
- `propel status` / `propel status --deep`
- `propel health --json`
- `propel update status`
- `propel cron add|list|runs|run`

Do not invent CLI flags or imply Propel enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Propel compaction:
