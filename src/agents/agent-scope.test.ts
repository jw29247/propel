...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: PropelConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: PropelConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: PropelConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: PropelConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/propel");
  });

  it("uses PROPEL_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "propel-home");
    vi.stubEnv("PROPEL_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as PropelConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".propel", "workspace"));
  });

  it("uses PROPEL_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "propel-home");
    vi.stubEnv("PROPEL_HOME", home);
    // Clear state dir so it falls back to PROPEL_HOME
    vi.stubEnv("PROPEL_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as PropelConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".propel", "agents", "main", "agent"));
  });
});
