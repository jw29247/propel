...
const GOOGLE_LIVE_SINGLE_KEY = "PROPEL_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "PROPEL_LIVE_ANTHROPIC_KEY",
    listVar: "PROPEL_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "PROPEL_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `PROPEL_LIVE_${base}_KEY`;
