...
vi.mock("./models-config.js", async (importOriginal) => {
  ...
  return {
    ...mod,
    ensurePropelModelsJson: vi.fn(async () => ({ wrote: false })),
  };
});
...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): PropelConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "propel-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "propel-workspace-"));
    const now = Date.now();
