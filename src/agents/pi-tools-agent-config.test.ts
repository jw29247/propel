...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "propel-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: PropelConfig) {
    return createPropelCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<PropelConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<PropelConfig["agents"]>["list"]>[number]["tools"];
  }): PropelConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createPropelCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createPropelCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createPropelCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createPropelCodingTools({
      config: cfg,
    ...
    const bobTools = createPropelCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createPropelCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createPropelCodingTools({
      config: cfg,
