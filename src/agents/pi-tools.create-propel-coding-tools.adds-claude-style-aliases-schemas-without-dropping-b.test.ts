...
const defaultTools = createPropelCodingTools({ senderIsOwner: true });
...
describe("createPropelCodingTools", () => {
  ...
  it("gates apply_patch behind tools.exec.applyPatch for OpenAI models", () => {
    ...
    const openAiTools = createPropelCodingTools({
      config,
    ...
    const anthropicTools = createPropelCodingTools({
      config,
  ...
  it("respects apply_patch allowModels", () => {
    ...
    const allowed = createPropelCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5.2",
    });
    ...
    const denied = createPropelCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
