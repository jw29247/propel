...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "propel.sandbox=1");
  args.push("--label", `propel.sessionKey=${params.scopeKey}`);
  args.push("--label", `propel.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `propel.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "propel.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`propel sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`propel sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("propel sandbox recreate --all");
}
