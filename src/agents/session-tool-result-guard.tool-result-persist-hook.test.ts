...
import { loadPropelPlugins } from "../plugins/loader.js";
...
describe("tool_result_persist hook", () => {
  ...
  it("loads tool_result_persist hooks without breaking persistence", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "propel-toolpersist-"));
...
describe("before_message_write hook", () => {
  it("continues persistence when a before_message_write hook throws", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "propel-before-write-"));
    process.env.PROPEL_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
