...
beforeAll(async () => {
  tempHome = await createTempHomeEnv("propel-skills-home-");
  await fs.mkdir(path.join(tempHome.home, ".propel", "agents", "main", "sessions"), {
    recursive: true,
  });
});
...
describe("applySkillEnvOverrides", () => {
  it("sets and restores env vars", async () => {
    ...
    await writeSkill({
      ...
      metadata: '{"propel":{"requires":{"env":["ENV_KEY"]},"primaryEnv":"ENV_KEY"}}',
    });
  ...
  it("blocks unsafe env overrides but allows declared secrets", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata:
        '{"propel":{"requires":{"env":["OPENAI_API_KEY","NODE_OPTIONS"]},"primaryEnv":"OPENAI_API_KEY"}}',
    });
  ...
  it("blocks dangerous host env overrides even when declared", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata: '{"propel":{"requires":{"env":["BASH_ENV","SHELL"]}}}',
    });
  ...
  it("allows required env overrides from snapshots", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata: '{"propel":{"requires":{"env":["OPENAI_API_KEY"]}}}',
    });
