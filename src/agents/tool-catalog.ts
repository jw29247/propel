...
function buildCoreToolGroupMap() {
  ...
  const propelTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInPropelGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:propel": propelTools,
