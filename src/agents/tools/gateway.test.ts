...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    propel: process.env.PROPEL_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.propel === undefined) {
      delete process.env.PROPEL_GATEWAY_TOKEN;
    } else {
      process.env.PROPEL_GATEWAY_TOKEN = envSnapshot.propel;
    }
  ...
  it("uses PROPEL_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.PROPEL_GATEWAY_TOKEN = "local-env-token";
