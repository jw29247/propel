...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): PropelConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as PropelConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): PropelConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as PropelConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: PropelConfig, accountId = "media") {
    await handleTelegramAction(
