...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses PROPEL_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      PROPEL_HOME: "/srv/propel-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/propel-home"), ".propel", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".propel", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("propel-workspace-");
