...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.PROPEL_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".propel", `workspace-${profile}`);
  }
  return path.join(home, ".propel", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".propel";
