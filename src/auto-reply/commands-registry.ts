...
export function isCommandEnabled(cfg: PropelConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: PropelConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as PropelConfig),
...
export function getCommandDetection(_cfg?: PropelConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: PropelConfig) {
