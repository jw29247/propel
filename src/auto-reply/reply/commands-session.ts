...
import { scheduleGatewaySigusr1Restart, triggerPropelRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Propel in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerPropelRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Propel via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
