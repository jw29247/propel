...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".propel-profile-decorated");
}
 ...
 */
export function decoratePropelProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_PROPEL_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_PROPEL_BROWSER_COLOR).toUpperCase();
