...
export function resolvePropelUserDataDir(profileName = DEFAULT_PROPEL_BROWSER_PROFILE_NAME) {
...
export async function launchPropelChrome(
  ...
  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_PROPEL_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decoratePropelProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
  ...
  try {
    ...
    log.warn(`propel browser clean-exit prefs failed: ${String(err)}`);
  }
...
export async function stopPropelChrome(running: RunningChrome, timeoutMs = 2500) {
