import type { BrowserConfig, BrowserProfileConfig, PropelConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_PROPEL_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "propel" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_PROPEL_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "propel" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_PROPEL_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_PROPEL_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Propel browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for propel/propel CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: PropelConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_PROPEL_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "propel";
