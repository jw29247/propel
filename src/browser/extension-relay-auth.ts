...
const RELAY_TOKEN_CONTEXT = "propel-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const PROPEL_RELAY_BROWSER = "Propel/extension-relay";

function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.PROPEL_GATEWAY_TOKEN?.trim() || process.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
export function resolveRelayAuthTokenForPort(port: number): string {
  ...
  throw new Error(
    "extension relay requires gateway auth token (set gateway.auth.token or PROPEL_GATEWAY_TOKEN)",
  );
}

export async function probeAuthenticatedPropelRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === PROPEL_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
