...
vi.mock("./chrome.js", () => ({
  resolvePropelUserDataDir: vi.fn(() => "/tmp/propel-test/propel/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolvePropelUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "propel-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolvePropelUserDataDir).mockReturnValue(userDataDir);
