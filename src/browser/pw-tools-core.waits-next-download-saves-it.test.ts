...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredPropelTmpDir: vi.fn(() => "/tmp/propel"),
}));
vi.mock("../infra/tmp-propel-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredPropelTmpDir.mockReturnValue("/tmp/propel");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredPropelTmpDir.mockReturnValue("/tmp/propel-preferred");
    ...
    const expectedDownloadsTail = `${path.join("tmp", "propel-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredPropelTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "propel-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "propel-preferred", "downloads")}${path.sep}`),
    );
  });
