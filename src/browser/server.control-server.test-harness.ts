...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.PROPEL_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/propel" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "propel",
        profiles: {
          propel: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchPropelChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolvePropelUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopPropelChrome: vi.fn(async () => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.PROPEL_GATEWAY_PORT;
  process.env.PROPEL_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.PROPEL_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.PROPEL_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.PROPEL_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.PROPEL_GATEWAY_PASSWORD;
  } else {
    process.env.PROPEL_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
