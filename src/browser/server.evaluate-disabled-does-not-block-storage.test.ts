  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          propel: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.PROPEL_GATEWAY_PORT;
    process.env.PROPEL_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.PROPEL_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.PROPEL_GATEWAY_PASSWORD;
