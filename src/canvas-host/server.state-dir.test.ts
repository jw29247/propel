...
describe("canvas host state dir defaults", () => {
  it("uses PROPEL_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("propel-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Propel Canvas");
      } finally {
        await handler.close();
      }
    });
