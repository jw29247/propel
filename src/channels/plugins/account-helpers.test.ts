...
function cfg(accounts?: Record<string, unknown> | null): PropelConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as PropelConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as PropelConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as PropelConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as PropelConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as PropelConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as PropelConfig)).toBe("default");
    });
  });
});
