...
function telegramCfg(): PropelConfig {
  return { channels: { telegram: { botToken: "tok" } } } as PropelConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: PropelConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as PropelConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as PropelConfig;
...
function createSignalAccountOverrideCfg(): PropelConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as PropelConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as PropelConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as PropelConfig,
        accountId: undefined,
        params: {
