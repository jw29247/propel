...
import { discoverPropelPlugins } from "../../plugins/discovery.js";
import type { PropelPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, PropelPackageManifest>>;
...
const ENV_CATALOG_PATHS = ["PROPEL_PLUGIN_CATALOG_PATHS", "PROPEL_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<PropelPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: PropelPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: PropelPackageManifest;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverPropelPlugins({ workspaceDir: options.workspaceDir });
