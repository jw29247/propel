...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: PropelConfig, policy: ChannelAccessPolicy) => PropelConfig;
  resolveAllowlist: (params: { cfg: PropelConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: PropelConfig; resolved: TResolved }) => PropelConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: PropelConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: PropelConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: PropelConfig, policy: ChannelAccessPolicy): PropelConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: PropelConfig, policy: ChannelAccessPolicy): PropelConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: PropelConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: PropelConfig; resolved: string[] }) => {
      calls.push("apply");
