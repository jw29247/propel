...
describe("runServiceRestart token drift", () => {
  ...
  beforeEach(() => {
    ...
    service.readCommand.mockResolvedValue({
      environment: { PROPEL_GATEWAY_TOKEN: "service-token" },
    });
    service.restart.mockResolvedValue(undefined);
    vi.unstubAllEnvs();
    vi.stubEnv("PROPEL_GATEWAY_TOKEN", "");
  ...
  it("uses env-first token precedence when checking drift", async () => {
    ...
    service.readCommand.mockResolvedValue({
      environment: { PROPEL_GATEWAY_TOKEN: "env-token" },
    });
    vi.stubEnv("PROPEL_GATEWAY_TOKEN", "env-token");
