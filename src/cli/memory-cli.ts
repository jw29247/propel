    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["propel memory status", "Show index and provider status."],
          ["propel memory index --force", "Force a full reindex."],
          ['propel memory search --query "deployment notes"', "Search indexed memory entries."],
          ["propel memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.propel.ai/cli/memory")}\n`,
    );
