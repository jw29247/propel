    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "propel node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["propel node status", "Check node host service status."],
          ["propel node install", "Install the node host service."],
          ["propel node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.propel.ai/cli/node")}\n`,
    );
