    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["propel nodes status", "List known nodes with live status."],
          ["propel nodes pairing pending", "Show pending node pairing requests."],
          ['propel nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["propel nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.propel.ai/cli/nodes")}\n`,
    );
