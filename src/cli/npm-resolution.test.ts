...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@propel/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@propel/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@propel/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@propel/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @propel/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@propel/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@propel/plugin-alpha@1.2.3",
        installPath: "/tmp/propel/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@propel/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @propel/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@propel/plugin-alpha@latest",
      true,
      "/tmp/propel/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@propel/plugin-alpha@latest",
      installPath: "/tmp/propel/extensions/alpha",
      version: "1.2.3",
