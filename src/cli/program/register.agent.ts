...
${formatHelpExamples([
  ['propel agent --to +15555550123 --message "status update"', "Start a new session."],
  ['propel agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'propel agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'propel agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['propel agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'propel agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.propel.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.propel.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "propel agents set-identity --identity-file ~/.propel/workspace/IDENTITY.md --agent main",
