...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "propel-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "propel-bundled-skills-test-"));
    process.env.PROPEL_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
