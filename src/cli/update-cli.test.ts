...
import type { PropelConfig, ConfigFileSnapshot } from "../config/types.propel.js";
...
vi.mock("../infra/propel-root.js", () => ({
  resolvePropelPackageRoot: vi.fn(),
}));
...
const { resolvePropelPackageRoot } = await import("../infra/propel-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/propel-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as PropelConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/propel-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolvePropelPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("propel-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolvePropelPackageRoot).mockClear();
    ...
    vi.mocked(resolvePropelPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("propel");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/propel-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "propel-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("Propel update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as PropelConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolvePropelPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("propel-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ PROPEL_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.PROPEL_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("propel-update-wizard");
    await withEnvAsync({ PROPEL_GIT_DIR: tempDir }, async () => {
      setTty(true);
