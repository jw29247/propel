...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Propel and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["propel update", "Update a source checkout (git)"],
        ["propel update --channel beta", "Switch to beta channel (git + npm)"],
        ["propel update --channel dev", "Switch to dev channel (git + npm)"],
        ["propel update --tag beta", "One-off update to a dist-tag or version"],
        ["propel update --dry-run", "Preview actions without changing anything"],
        ["propel update --no-restart", "Update without restarting the service"],
        ["propel update --json", "Output result as JSON"],
        ["propel update --yes", "Non-interactive (accept downgrade prompts)"],
        ["propel update wizard", "Interactive update wizard"],
        ["propel --update", "Shorthand for propel update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run propel update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.propel.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["propel update status", "Show channel + version status."],
          ["propel update status --json", "JSON output."],
          ["propel update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.propel.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
