...
function resolveAgentName(cfg: PropelConfig, agentId: string) {
...
function resolveAgentModel(cfg: PropelConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: PropelConfig): AgentSummary[] {
