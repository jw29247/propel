...
const SEARCH_TOOL = "https://docs.propel.ai/mcp.SearchPropel";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.propel.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('propel docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.propel.ai/");
      runtime.log(`Search: ${formatCliCommand('propel docs "your query"')}`);
    }
    return;
  }
