...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "PROPEL_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("PROPEL_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv PROPEL_GATEWAY_TOKEN");
    expect(message).not.toContain("PROPEL_GATEWAY_PASSWORD");
  });
