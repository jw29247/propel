...
import type { PropelConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: PropelConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("propel security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.propel/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("propel approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.propel.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("propel configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("propel config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("propel doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("propel config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('propel config set session.dmScope "per-channel-peer"') +
