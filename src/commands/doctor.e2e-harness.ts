...
export const resolvePropelPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadPropelPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolvePropelPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.PROPEL_STATE_DIR;
  originalUpdateInProgress = process.env.PROPEL_UPDATE_IN_PROGRESS;
  process.env.PROPEL_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "propel-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.PROPEL_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.PROPEL_UPDATE_IN_PROGRESS;
  } else {
    process.env.PROPEL_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
