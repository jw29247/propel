...
function resolveMode(cfg: PropelConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolvePropelPackageRoot({
  ...
  let cfg: PropelConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("propel configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("propel config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("propel setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("propel doctor --fix")}" to apply changes.`);
  }
