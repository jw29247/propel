...
type PromptDefaultModelResult = { model?: string; config?: PropelConfig };
...
function resolveConfiguredModelRaw(cfg: PropelConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: PropelConfig): string[] {
...
export function applyPrimaryModel(cfg: PropelConfig, model: string): PropelConfig {
...
export function applyModelAllowlist(cfg: PropelConfig, models: string[]): PropelConfig {
  const defaults = cfg.agents?.defaults;
