...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolvePropelAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("propel models auth setup-token")}\` or \`${formatCliCommand("propel configure")}\`.`
          : `Run \`${formatCliCommand("propel configure")}\` or set an API key env var.`;
