...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolvePropelAgentDir: vi.fn().mockReturnValue("/tmp/propel-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/propel-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/propel-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolvePropelAgentDir: mocks.resolvePropelAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/propel-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolvePropelAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/propel-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/propel-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/propel-agent-custom");
