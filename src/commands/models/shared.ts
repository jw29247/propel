...
export async function loadValidConfigOrThrow(): Promise<PropelConfig> {
...
export async function updateConfig(
  mutator: (cfg: PropelConfig) => PropelConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: PropelConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: PropelConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("propel agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
