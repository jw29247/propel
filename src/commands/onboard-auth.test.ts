...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under PROPEL_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("propel-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "propel-oauth-sync-"));
    ...
    process.env.PROPEL_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "propel-oauth-nosync-"));
    process.env.PROPEL_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside PROPEL_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "propel-oauth-external-"));
    process.env.PROPEL_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* PROPEL_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to PROPEL_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("propel-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
