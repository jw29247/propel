...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: PropelConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: PropelConfig) => { changed: boolean; next: PropelConfig };
  defaultModel: string;
  overrideConfig: PropelConfig;
  alreadyDefaultConfig: PropelConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: PropelConfig) => config,
      applyDefaultConfig: (config: PropelConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: PropelConfig = { agents: { defaults: {} } };
