...
beforeAll(() => {
  envSnapshot = captureEnv(["PROPEL_PROFILE"]);
  process.env.PROPEL_PROFILE = "isolated";
});
...
vi.mock("../infra/propel-root.js", () => ({
  resolvePropelPackageRoot: vi.fn().mockResolvedValue("/tmp/propel"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/propel/pnpm-lock.yaml",
      markerPath: "/tmp/propel/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("Propel status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("propel status --all") ||
          l.includes("propel --profile isolated status --all") ||
          l.includes("propel status --all") ||
          l.includes("propel --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.PROPEL_GATEWAY_TOKEN = "abcd1234";
    try {
