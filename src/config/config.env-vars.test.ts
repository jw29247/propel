...
describe("config env vars", () => {
  it("applies env vars from env block when missing", async () => {
    await withEnvOverride({ OPENROUTER_API_KEY: undefined }, async () => {
      applyConfigEnvVars({ env: { vars: { OPENROUTER_API_KEY: "config-key" } } } as PropelConfig);
  ...
  it("applies env vars from env.vars when missing", async () => {
    await withEnvOverride({ GROQ_API_KEY: undefined }, async () => {
      applyConfigEnvVars({ env: { vars: { GROQ_API_KEY: "gsk-config" } } } as PropelConfig);
  ...
  it("blocks dangerous startup env vars from config env", async () => {
    await withEnvOverride(
      ...
      async () => {
        ...
        const entries = collectConfigRuntimeEnvVars(config as PropelConfig);
        ...
        applyConfigEnvVars(config as PropelConfig);
  ...
  it("loads ${VAR} substitutions from ~/.propel/.env on repeated runtime loads", async () => {
    await withTempHome(async (_home) => {
      await withEnvOverride({ BRAVE_API_KEY: undefined }, async () => {
        const stateDir = process.env.PROPEL_STATE_DIR?.trim();
        if (!stateDir) {
          throw new Error("Expected PROPEL_STATE_DIR to be set by withTempHome");
        }
        await fs.mkdir(stateDir, { recursive: true });
        await fs.writeFile(path.join(stateDir, ".env"), "BRAVE_API_KEY=from-dotenv\n", "utf-8");

        const config: PropelConfig = {
          tools: {
            web: {
              search: {
        ...
        const first = resolveConfigEnvVars(config, process.env) as PropelConfig;
        expect(first.tools?.web?.search?.apiKey).toBe("from-dotenv");

        delete process.env.BRAVE_API_KEY;
        loadDotEnv({ quiet: true });
        const second = resolveConfigEnvVars(config, process.env) as PropelConfig;
