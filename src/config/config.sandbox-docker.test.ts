...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("propel-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "propel-sandbox-browser-global" },
      agentBrowser: { network: "propel-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("propel-sandbox-browser-agent");
  });
