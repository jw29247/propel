...
import type { PropelConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { comparePropelVersions } from "./version.js";
...
function unsetPathForWrite(
  root: PropelConfig,
  pathSegments: string[],
): { changed: boolean; next: PropelConfig } {
...
function coerceConfig(value: unknown): PropelConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as PropelConfig;
}
...
function stampConfigVersion(cfg: PropelConfig): PropelConfig {
...
function warnIfConfigFromFuture(cfg: PropelConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = comparePropelVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Propel (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as PropelConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): PropelConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as PropelConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: PropelConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as PropelConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.PROPEL_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.PROPEL_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.PROPEL_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.PROPEL_WATCH_SESSION === "string" &&
        deps.env.PROPEL_WATCH_SESSION.trim().length > 0
          ? deps.env.PROPEL_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.PROPEL_WATCH_COMMAND === "string" &&
        deps.env.PROPEL_WATCH_COMMAND.trim().length > 0
          ? deps.env.PROPEL_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `PROPEL_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.PROPEL_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.PROPEL_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): PropelConfig {
