...
import type { ConfigFileSnapshot } from "./types.propel.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.propel/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/propel/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/propel/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/propel/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/propel/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = PropelSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "PropelConfig";
    const hints = mapSensitivePaths(PropelSchema, "", {});
