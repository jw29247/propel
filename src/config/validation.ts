...
import type { PropelConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: PropelConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: PropelConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = PropelSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as PropelConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as PropelConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as PropelConfig,
  };
}
