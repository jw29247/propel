...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "propel-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses PROPEL_HOME for tilde expansion", () => {
    vi.stubEnv("PROPEL_HOME", "/srv/propel-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/propel-home", "cron", "jobs.json"));
  });
});
