...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.PROPEL_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service PROPEL_GATEWAY_TOKEN does not match gateway.auth.token in propel.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `propel gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
