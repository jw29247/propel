...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.PROPEL_GATEWAY_PORT).toBe("18789");
    expect(env.PROPEL_GATEWAY_TOKEN).toBe("secret");
    expect(env.PROPEL_SERVICE_MARKER).toBe("propel");
    expect(env.PROPEL_SERVICE_KIND).toBe("gateway");
    expect(typeof env.PROPEL_SERVICE_VERSION).toBe("string");
    expect(env.PROPEL_SYSTEMD_UNIT).toBe("propel-gateway.service");
    if (process.platform === "darwin") {
      expect(env.PROPEL_LAUNCHD_LABEL).toBe("ai.propel.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", PROPEL_PROFILE: "work" },
      port: 18789,
    });
    expect(env.PROPEL_SYSTEMD_UNIT).toBe("propel-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.PROPEL_LAUNCHD_LABEL).toBe("ai.propel.work");
    }
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".propel"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", PROPEL_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".propel-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", PROPEL_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".propel"));
  });

  it("uses PROPEL_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", PROPEL_STATE_DIR: "/var/lib/propel" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/propel"));
  });

  it("expands ~ in PROPEL_STATE_DIR", () => {
    const env = { HOME: "/Users/test", PROPEL_STATE_DIR: "~/propel-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/propel-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { PROPEL_STATE_DIR: "C:\\State\\propel" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\propel");
  });
});
