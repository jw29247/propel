...
const resolveStorePath = vi.fn(() => "/tmp/propel-discord-process-test-sessions.json");
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/propel-discord-process-test-sessions.json");
  ...
  async function createBlockModeContext() {
    return await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/propel-discord-process-test-sessions.json" },
        channels: {
          discord: {
