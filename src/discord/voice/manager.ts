...
function resolveVoiceTtsConfig(params: { cfg: PropelConfig; override?: TtsConfig }): {
  cfg: PropelConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredPropelTmpDir(), "discord-voice-"));
