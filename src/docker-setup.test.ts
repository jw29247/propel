...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "propel-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    PROPEL_GATEWAY_TOKEN: "test-token",
    PROPEL_CONFIG_DIR: join(sandbox.rootDir, "config"),
    PROPEL_WORKSPACE_DIR: join(sandbox.rootDir, "propel"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      PROPEL_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      PROPEL_EXTRA_MOUNTS: undefined,
      PROPEL_HOME_VOLUME: "propel-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("PROPEL_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("PROPEL_EXTRA_MOUNTS=");
    expect(envFile).toContain("PROPEL_HOME_VOLUME=propel-home");
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("propel-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("propel-home:");
    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg PROPEL_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
  ...
  it("precreates config identity dir for CLI device auth writes", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      PROPEL_CONFIG_DIR: configDir,
      PROPEL_WORKSPACE_DIR: workspaceDir,
    });
  ...
  it("rejects injected multiline PROPEL_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      PROPEL_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("PROPEL_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid PROPEL_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      PROPEL_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid PROPEL_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      PROPEL_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("PROPEL_HOME_VOLUME must match");
  });
