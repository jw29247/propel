...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "propel.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "propel.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.PROPEL_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.PROPEL_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.PROPEL_NODE_OPTIONS_READY = "1";
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[propel] ${parsed.error}`);
      process.exit(2);
    }
