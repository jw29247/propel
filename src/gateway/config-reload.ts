...
import type { PropelConfig, ConfigFileSnapshot, GatewayReloadMode } from "../config/config.js";
...
export function resolveGatewayReloadSettings(cfg: PropelConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: PropelConfig;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: PropelConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: PropelConfig) => void;
  log: {
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: PropelConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: PropelConfig) => {
