...
export function resolveHooksConfig(cfg: PropelConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: PropelConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-propel-token"] === "string"
      ? req.headers["x-propel-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
