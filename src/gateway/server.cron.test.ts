...
async function cleanupCronTestRun(params: {
  ...
  process.env.PROPEL_SKIP_CRON = params.prevSkipCron;
}

describe("gateway server cron", () => {
  test("handles cron CRUD, normalization, and patch semantics", { timeout: 120_000 }, async () => {
    const prevSkipCron = process.env.PROPEL_SKIP_CRON;
    process.env.PROPEL_SKIP_CRON = "0";
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "propel-gw-cron-"));
  ...
  test("writes cron run history and auto-runs due jobs", async () => {
    const prevSkipCron = process.env.PROPEL_SKIP_CRON;
    process.env.PROPEL_SKIP_CRON = "0";
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "propel-gw-cron-log-"));
    testState.cronStorePath = path.join(dir, "cron", "jobs.json");
    testState.cronEnabled = undefined;
    await fs.mkdir(path.dirname(testState.cronStorePath), { recursive: true });
    await fs.writeFile(testState.cronStorePath, JSON.stringify({ version: 1, jobs: [] }));
  ...
  test("posts webhooks for delivery mode and legacy notify fallback only when summary exists", async () => {
    const prevSkipCron = process.env.PROPEL_SKIP_CRON;
    process.env.PROPEL_SKIP_CRON = "0";
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "propel-gw-cron-webhook-"));
    testState.cronStorePath = path.join(dir, "cron", "jobs.json");
    testState.cronEnabled = false;
    await fs.mkdir(path.dirname(testState.cronStorePath), { recursive: true });
