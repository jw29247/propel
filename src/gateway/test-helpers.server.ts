...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "PROPEL_STATE_DIR",
  "PROPEL_CONFIG_PATH",
  "PROPEL_SKIP_BROWSER_CONTROL_SERVER",
  "PROPEL_SKIP_GMAIL_WATCHER",
  "PROPEL_SKIP_CANVAS_HOST",
  "PROPEL_BUNDLED_PLUGINS_DIR",
  "PROPEL_SKIP_CHANNELS",
  "PROPEL_SKIP_PROVIDERS",
  "PROPEL_SKIP_CRON",
  "PROPEL_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "propel-gateway-home-"));
...
function applyGatewaySkipEnv() {
  process.env.PROPEL_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.PROPEL_SKIP_GMAIL_WATCHER = "1";
  process.env.PROPEL_SKIP_CANVAS_HOST = "1";
  process.env.PROPEL_SKIP_CHANNELS = "1";
  process.env.PROPEL_SKIP_PROVIDERS = "1";
  process.env.PROPEL_SKIP_CRON = "1";
  process.env.PROPEL_TEST_MINIMAL_GATEWAY = "1";
  process.env.PROPEL_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "propel-test-no-bundled-extensions")
    : "propel-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "propel-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".propel-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__propelTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__propelTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["PROPEL_GATEWAY_TOKEN"]);
  const prev = process.env.PROPEL_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.PROPEL_GATEWAY_TOKEN = fallbackToken;
  }
