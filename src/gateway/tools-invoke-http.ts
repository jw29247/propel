...
import { createPropelTools } from "../agents/propel-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-propel-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-propel-account-id")?.trim() || undefined;
  ...
  const allTools = createPropelTools({
