...
This directory contains hooks that ship with Propel. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.propel/propel.json`:
...
2. Restart gateway: `pkill -9 -f 'propel.*gateway' && pnpm propel gateway`
3. Enable the hook: `propel hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.propel.ai/automation/hooks
