...
function createBootstrapExtraConfig(paths: string[]): PropelConfig {
  return {
    hooks: {
      internal: {
        entries: {
...
describe("bootstrap-extra-files hook", () => {
  it("appends extra bootstrap files from configured patterns", async () => {
    const tempDir = await makeTempWorkspace("propel-bootstrap-extra-");
  ...
  it("re-applies subagent bootstrap allowlist after extras are added", async () => {
    const tempDir = await makeTempWorkspace("propel-bootstrap-extra-subagent-");
