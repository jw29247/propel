...
homepage: https://docs.propel.ai/automation/hooks#command-logger
metadata:
  {
    "propel":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.propel/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.propel/logs/commands.log ~/.propel/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/propel`:
   ```
   /home/username/.propel/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
