...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => PropelConfig;
  action?: "new" | "reset";
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("propel-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): PropelConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
