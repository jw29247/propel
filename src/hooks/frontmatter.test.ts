...
`;
    ...
    expect(parsed.propel.emoji).toBe("💾");
    expect(parsed.propel.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.propel.emoji).toBe("📝");
    expect(parsed.propel.events).toEqual(["command"]);
    expect(parsed.propel.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.propel.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"propel": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"propel": {"events": ["test"]}}');
  });
...
describe("resolvePropelMetadata", () => {
  it("extracts propel metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolvePropelMetadata(frontmatter);
  ...
  it("returns undefined when propel key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        propel: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Propel" },
            { id: "npm", kind: "npm", package: "@propel/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@propel/hook");
  });
...
`;
    ...
    const propel = resolvePropelMetadata(frontmatter);
    expect(propel).toBeDefined();
    expect(propel?.emoji).toBe("💾");
    expect(propel?.events).toEqual(["command:new", "command:reset"]);
    expect(propel?.requires?.config).toEqual(["workspace.dir"]);
    expect(propel?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const propel = resolvePropelMetadata(frontmatter);
    expect(propel?.emoji).toBe("disk");
    expect(propel?.events).toEqual(["command:new"]);
  });
});
