...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "propel-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["PROPEL_BUNDLED_HOOKS_DIR"]);
    process.env.PROPEL_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
