...
vi.mock("./propel-root.js", () => ({
  resolvePropelPackageRoot: vi.fn(async () => null),
  resolvePropelPackageRootSync: vi.fn(() => null),
}));
...
let propelRoot: typeof import("./propel-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    propelRoot = await import("./propel-root.js");
  });
  ...
  it("uses resolvePropelPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/propel");
    (
      propelRoot.resolvePropelPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/propel"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "propel" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "propel.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-propel");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/propel-bundle");
    (
      propelRoot.resolvePropelPackageRootSync as unknown as ReturnType<typeof vi.fn>
