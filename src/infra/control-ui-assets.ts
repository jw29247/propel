...
import { resolvePropelPackageRoot, resolvePropelPackageRootSync } from "./propel-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolvePropelPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "propel" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolvePropelPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
