...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __propelDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__propelDiagnosticEventsState) {
    globalStore.__propelDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__propelDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: PropelConfig): boolean {
