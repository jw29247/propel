...
beforeAll(async () => {
  ...
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "propel-heartbeat-suite-"));
});
...
describe("resolveHeartbeatPrompt", () => {
  it("uses default or trimmed override prompts", () => {
    const cases = [
      { cfg: {} as PropelConfig, expected: HEARTBEAT_PROMPT },
      {
        cfg: {
...
describe("runHeartbeatOnce", () => {
  ...
  it("resolves configured and forced session key overrides", async () => {
    ...
    try {
      const cases = typedCases<{
        ...
        peerId: string;
        message: string;
        applyOverride: (params: { cfg: PropelConfig; sessionKey: string }) => void;
      ...
      }>([
        {
          ...
          message: "Group alert",
          applyOverride: ({ cfg, sessionKey }: { cfg: PropelConfig; sessionKey: string }) => {
  ...
  it("loads the default agent session from templated stores", async () => {
    const tmpDir = await createCaseDir("propel-hb");
