...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: PropelConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: PropelConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: PropelConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: PropelConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: PropelConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: PropelConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: PropelConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
