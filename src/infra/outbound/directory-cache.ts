...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: PropelConfig | null = null;
  ...
  get(key: string, cfg: PropelConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: PropelConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: PropelConfig): void {
