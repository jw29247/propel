...
let ensurePropelCliOnPath: typeof import("./path-env.js").ensurePropelCliOnPath;

describe("ensurePropelCliOnPath", () => {
  const envKeys = [
    "PATH",
    "PROPEL_PATH_BOOTSTRAPPED",
    "PROPEL_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensurePropelCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling propel exists", () => {
    const tmp = abs("/tmp/propel-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "propel");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.PROPEL_PATH_BOOTSTRAPPED;

    ensurePropelCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.PROPEL_PATH_BOOTSTRAPPED = "1";
    ensurePropelCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/propel-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "propel");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/propel-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "propel");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "propel");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/propel-path/case-linuxbrew");
