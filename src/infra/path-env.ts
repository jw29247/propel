...
function candidateBinDirs(opts: EnsurePropelPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `propel` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "propel");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.PROPEL_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "propel"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `propel` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensurePropelCliOnPath(opts: EnsurePropelPathOpts = {}) {
  if (isTruthyEnvValue(process.env.PROPEL_PATH_BOOTSTRAPPED)) {
    return;
  }
