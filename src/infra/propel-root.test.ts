...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__propel_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "propel-root");
...
describe("resolvePropelPackageRoot", () => {
  let resolvePropelPackageRoot: typeof import("./propel-root.js").resolvePropelPackageRoot;
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "propel");
    const pkgRoot = path.join(project, "node_modules", "propel");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "propel" }));

    expect(resolvePropelPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "propel");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "propel.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "propel" }));

    expect(resolvePropelPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolvePropelPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-propel package roots", async () => {
    const pkgRoot = fx("not-propel");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-propel" }));

    expect(resolvePropelPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolvePropelPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolvePropelPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
