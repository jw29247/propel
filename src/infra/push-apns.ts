...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.PROPEL_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.PROPEL_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set PROPEL_APNS_TEAM_ID and PROPEL_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.PROPEL_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.PROPEL_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.PROPEL_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set PROPEL_APNS_PRIVATE_KEY_P8 or PROPEL_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading PROPEL_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createPropelPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    propel: createPropelPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
