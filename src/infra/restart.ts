...
export function triggerPropelRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.PROPEL_SYSTEMD_UNIT,
        process.env.PROPEL_PROFILE,
      );
  ...
  const label =
    process.env.PROPEL_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.PROPEL_PROFILE);
