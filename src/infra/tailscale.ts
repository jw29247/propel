...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.PROPEL_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Propel. You can keep running the web gateway without it: \`${formatCliCommand("propel gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
