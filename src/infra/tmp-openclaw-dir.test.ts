...
import { POSIX_PROPEL_TMP_DIR, resolvePreferredPropelTmpDir } from "./tmp-propel-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredPropelTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `propel-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredPropelTmpDir({
    accessSync,
...
describe("resolvePreferredPropelTmpDir", () => {
  it("prefers /tmp/propel when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_PROPEL_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/propel when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_PROPEL_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/propel when /tmp/propel is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/propel when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/propel is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/propel is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/propel is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
