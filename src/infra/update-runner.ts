...
const DEFAULT_PACKAGE_NAME = "propel";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "propel-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "propel.mjs");
    ...
    const doctorStep = await runStep(
      step("propel doctor", doctorArgv, gitRoot, { PROPEL_UPDATE_IN_PROGRESS: "1" }),
    );
