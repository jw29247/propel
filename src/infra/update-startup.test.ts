...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "propel-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["PROPEL_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.PROPEL_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolvePropelPackageRoot } = await import("./propel-root.js"));
  ...
  function mockPackageUpdateStatus(tag = "latest", version = "2.0.0") {
    vi.mocked(resolvePropelPackageRoot).mockResolvedValue("/opt/propel");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/propel/dist/entry.js"];
    try {
      await runGatewayUpdateCheck({
        cfg: {
          update: {
