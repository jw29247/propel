...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.PROPEL_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*propel-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "propel";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "propel");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "propel-gw");
  ...
  records.push(`_propel-gw._tcp IN PTR ${instanceLabel}._propel-gw._tcp`);
  records.push(`${instanceLabel}._propel-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._propel-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; propel-content-hash: ${contentHash}\n${contentBody}`;
}
