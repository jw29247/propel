...
import type { PropelConfig } from "../config/types.js";
...
export const DEFAULT_LOG_DIR = resolvePreferredPropelTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "propel.log"); // legacy single-file path

const LOG_PREFIX = "propel";
...
function resolveSettings(): ResolvedSettings {
  let cfg: PropelConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => PropelConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.PROPEL_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[propel] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
