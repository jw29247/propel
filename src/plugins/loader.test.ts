...
import { loadPropelPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `propel-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.PROPEL_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "propel.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.PROPEL_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadPropelPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.PROPEL_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.PROPEL_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadPropelPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadPropelPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadPropelPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.PROPEL_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("warns when loaded non-bundled plugin has no install/load-path provenance", () => {
    ...
    withEnv({ PROPEL_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
