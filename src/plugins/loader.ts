...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: PropelPluginDefinition;
  register?: PropelPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as PropelPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as PropelPluginDefinition;
...
export function loadPropelPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverPropelPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "propel/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "propel/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: PropelPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as PropelPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
