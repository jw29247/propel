...
function createSinglePluginWithEmptySlotsConfig(): PropelConfig {
  return {
    plugins: {
...
function createSingleNpmInstallConfig(installPath: string): PropelConfig {
  return {
    plugins: {
      entries: createSinglePluginEntries(),
      installs: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "propel-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        source: "npm",
        spec: "my-plugin@1.0.0",
        installPath: "/tmp/not-propel-extensions/my-plugin",
      },
      extensionsDir,
    });
