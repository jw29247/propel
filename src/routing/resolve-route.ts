...
function listAgents(cfg: PropelConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: PropelConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: PropelConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<PropelConfig, EvaluatedBindingsCache>();
