...
function summarizeGroupPolicy(cfg: PropelConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: PropelConfig): boolean {
...
function collectModels(cfg: PropelConfig): ModelRef[] {
  const out: ModelRef[] = [];
  addModel(
    out,
...
function listKnownNodeCommands(cfg: PropelConfig): Set<string> {
  const baseCfg: PropelConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: PropelConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: PropelConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: PropelConfig): boolean {
...
function isBrowserEnabled(cfg: PropelConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: PropelConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: PropelConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep PROPEL_STATE_DIR on a local-only volume and re-run "${formatCliCommand("propel security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: PropelConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer PROPEL_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const propelGatewayToken =
    typeof env.PROPEL_GATEWAY_TOKEN === "string" && env.PROPEL_GATEWAY_TOKEN.trim()
      ? env.PROPEL_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-propel-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: PropelConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: PropelConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  ...
  if (browserExposurePaths.length > 0) {
    findings.push({
      ...
      remediation:
        "Set sandbox.browser.network to a dedicated bridge network (recommended default: propel-sandbox-browser), " +
...
export function collectNodeDenyCommandPatternFindings(cfg: PropelConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: PropelConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: PropelConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: PropelConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
