...
describe("shared/frontmatter", () => {
  ...
  test("resolvePropelManifestBlock parses JSON5 metadata and picks propel block", () => {
    const frontmatter = {
      metadata: "{ propel: { foo: 1, bar: 'baz' } }",
    };
    expect(resolvePropelManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolvePropelManifestBlock returns undefined for invalid input", () => {
    expect(resolvePropelManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolvePropelManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolvePropelManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
