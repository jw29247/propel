...
describe("resolveGatewayConnection", () => {
  ...
  it("uses PROPEL_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ PROPEL_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers PROPEL_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ PROPEL_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
