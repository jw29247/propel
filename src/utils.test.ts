...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("propel-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("propel-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("propel-lid-a-", (first) => {
      withTempDirSync("propel-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.propel when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "propel-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers PROPEL_HOME over HOME", () => {
    vi.stubEnv("PROPEL_HOME", "/srv/propel-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/propel-home"));
...
describe("shortenHomePath", () => {
  it("uses $PROPEL_HOME prefix when PROPEL_HOME is set", () => {
    vi.stubEnv("PROPEL_HOME", "/srv/propel-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/propel-home")}/.propel/propel.json`)).toBe(
      "$PROPEL_HOME/.propel/propel.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $PROPEL_HOME replacement when PROPEL_HOME is set", () => {
    vi.stubEnv("PROPEL_HOME", "/srv/propel-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/propel-home")}/.propel/propel.json`),
    ).toBe("config: $PROPEL_HOME/.propel/propel.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/propel")).toBe(path.resolve(os.homedir(), "propel"));
  });
  ...
  it("prefers PROPEL_HOME for tilde expansion", () => {
    vi.stubEnv("PROPEL_HOME", "/srv/propel-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/propel")).toBe(path.resolve("/srv/propel-home", "propel"));
