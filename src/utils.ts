...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".propel");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.PROPEL_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$PROPEL_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via PROPEL_STATE_DIR.
