...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "propel-version-"));
  try {
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "propel", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "propel", version: "2.3.4" });
  ...
  it("ignores non-propel package and blank build-info versions", async () => {
    await withTempDir(async (root) => {
  ...
  it("prefers PROPEL_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        PROPEL_VERSION: "9.9.9",
        PROPEL_SERVICE_VERSION: "2.2.2",
