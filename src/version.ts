...
declare const __PROPEL_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "propel";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["PROPEL_VERSION"],
      env["PROPEL_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current Propel version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION =
  (typeof __PROPEL_VERSION__ === "string" && __PROPEL_VERSION__) ||
  process.env.PROPEL_BUNDLED_VERSION ||
