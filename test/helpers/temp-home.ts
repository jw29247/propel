...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    propelHome: process.env.PROPEL_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("PROPEL_HOME", snapshot.propelHome);
  restoreKey("PROPEL_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked PROPEL_HOME.
  delete process.env.PROPEL_HOME;
  process.env.PROPEL_STATE_DIR = path.join(base, ".propel");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "propel-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".propel", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
