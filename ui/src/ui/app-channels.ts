...
export async function handleWhatsAppStart(host: PropelApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: PropelApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: PropelApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: PropelApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: PropelApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: PropelApp): string {
...
function resolveGatewayHttpAuthHeader(host: PropelApp): string | null {
...
function buildGatewayHttpHeaders(host: PropelApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: PropelApp) {
...
export function handleNostrProfileToggleAdvanced(host: PropelApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: PropelApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: PropelApp) {
  const state = host.nostrProfileFormState;
