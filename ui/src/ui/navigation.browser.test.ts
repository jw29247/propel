...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/propel/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/propel");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/propel/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__PROPEL_CONTROL_UI_BASE_PATH__ = "/propel";
    const app = mountApp("/propel/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/propel");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/propel/sessions");
  });
